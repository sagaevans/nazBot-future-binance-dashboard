# ==========================================
# nazBot Sniper System [BETA v2.1] - PRO EDITION
# FILE: bot_logic.py
# FUNGSI: Saldo Riil, Excel ROE, Gold 10x, & RSI Anti-Pucuk
# ==========================================
...
# --- PENGATURAN KHUSUS GOLD (V2.1) ---
GOLD_LEVERAGE = 10
GOLD_MARGIN = 20.0

# --- KONFIGURASI 6-STAGE SURVIVAL DCA ---
DCA_TRIGGERS = {1: -50.0, 2: -100.0, 3: -200.0, 4: -400.0, 5: -800.0, 6: -1200.0}
...
_active_margins: Dict[str, float] = {} # Memori untuk menyimpan margin aktual per koin

_client = Client(API_KEY, API_SECRET, testnet=True) # Set False untuk real
...
def get_initial_balance() -> float:
    """Mengambil Start Balance dinamis. Jika ledger kosong, ambil riil dari Binance."""
    is_new = not os.path.exists(LEDGER_FILE) or os.path.getsize(LEDGER_FILE) == 0
    
    # Simpan di file kecil agar persisten
    if is_new:
        start_bal = get_binance_balance()
        if start_bal == 0: start_bal = 5000.0 # Fallback safety
        ...
        return 5000.0 # Fallback jika file hilang
...
def get_last_ledger_totals() -> float:
    """Mengambil akumulasi Total ROE terakhir dari Ledger (Excel Style)."""
...
def catat_transaksi_v2(symbol: str, pnl_usd: float, roe_percent: float):
    """Mencatat Ledger menggunakan Penjumlahan Linear Excel"""
    prev_tot_roe = get_last_ledger_totals()
    ...
    log_line = (f"{now} | {symbol} | {pnl_usd:+.2f} | {roe_percent:+.2f}% | "
                f"{pnl_usd:+.2f} | {new_tot_roe:+.2f}% | "
                f"{current_balance:.2f} | {growth_pct:+.2f}%\n")
    
    is_new = not os.path.exists(LEDGER_FILE) or os.path.getsize(LEDGER_FILE) == 0
    with open(LEDGER_FILE, 'a') as f:
        if is_new:
            f.write("TIME | PAIR | PROFIT $ | ROE % | REALIZED $ | TOTAL ROE % | SALDO BINANCE | GROWTH %\n")
            f.write("-" * 115 + "\n")
        f.write(log_line)
...
def _get_dynamic_leverage_and_margin(symbol: str, target_margin: float) -> Tuple[int, float]:
    """Pemisahan Kasta: Emas pakai Lev 10x, Altcoin Lev 50x"""
    if symbol in GOLD_SET:
        return GOLD_LEVERAGE, GOLD_MARGIN

    try:
        _api_call(_client.futures_change_leverage, symbol=symbol, leverage=TARGET_LEVERAGE)
        ...
        if e.code == -4028:
            ...
            except: return TARGET_LEVERAGE, target_margin
        return TARGET_LEVERAGE, target_margin
    except: return TARGET_LEVERAGE, target_margin
...
# ========== PRO SIGNAL LOGIC (V2.1 ANTI-PUCUK) ==========
def get_adaptive_signal(symbol: str, tf: str, is_vip: bool) -> Optional[dict]:
    try:
        ...
        # --- 1. FILTER RSI (ANTI-PIPPIN / ANTI-PUCUK) ---
        if rsi.iat[idx_curr] > 70:
            return None 

        # --- 2. FILTER TREND (EMA 200) ---
        if c_close < ema200.iat[idx_curr]:
            return None
        ...
        dynamic_floors = [t for t in [ema200.iat[idx_curr], ma99.iat[idx_curr], bb.bollinger_lband().iat[idx_curr]] if t < c_close]
        ...
        if hit_dynamic:
            return {'side': 'LONG', 'reason': 'Dynamic Wall + Uptrend'}
        return None
    except: return None
...
def execute_order(symbol: str, side: str, position_side: str, margin_to_use: float, is_dca: bool = False) -> bool:
    try:
        ...
        qty_step, min_qty = float(f['LOT_SIZE']['stepSize']), float(f['LOT_SIZE']['minQty'])
...
def _monitor_positions(positions: List[dict]):
    ...
    for p in positions:
        ...
        _active_margins[symbol] = current_margin # MEMORY UPDATE: Catat margin aktual riil
        ...
        if roe_percent <= DCA_TRIGGERS[6] and current_margin < (base_adj * 15.0): 
            tahap = "6 (HARD RECOVERY)"
            suntikan = base_adj * 4.0 
        elif roe_percent <= DCA_TRIGGERS[5] and current_margin < (base_adj * 8.0):
            tahap = "5 (LAST STAND)"
            suntikan = base_adj * 4.0 
        elif roe_percent <= DCA_TRIGGERS[4] and current_margin < (base_adj * 4.0):
            tahap = "4 (SURVIVAL MODE)"
            suntikan = base_adj * 2.0
        elif roe_percent <= DCA_TRIGGERS[3] and current_margin < (base_adj * 2.0):
            tahap = "3 (REVERSAL AREA)"
            suntikan = base_adj * 1.0
        elif roe_percent <= DCA_TRIGGERS[2] and current_margin < (base_adj * 1.5):
            tahap = "2 (KOREKSI NORMAL)"
            suntikan = base_adj * 0.5
        elif roe_percent <= DCA_TRIGGERS[1] and current_margin < (base_adj * 1.0):
            tahap = "1 (KOREKSI TIPIS)"
            suntikan = base_adj * 0.5

        if suntikan > 0:
            logger.info(f"💉 DCA TAHAP {tahap} [{symbol}] Terpicu di ROE {roe_percent:.2f}% | Target Suntikan: ${suntikan:.2f}")
            execute_order(symbol, 'BUY', 'LONG', suntikan, is_dca=True)
...
def run_bot(stop_event: threading.Event) -> None:
    global _stop_event, TOTAL_CLOSED_ROE_PERCENT, TOTAL_SUCCESS_TRADES, CLOSED_HISTORY, _coin_escalation_level, _active_margins
    _stop_event = stop_event
    setup_account_environment()

    # Trigger initial balance cache
    get_initial_balance()
            ...
            try:
                ...
                if not first_run:
                    ...
                    for k in closed_keys:
                        ...
                        # Ambil margin aktual riil, BUKAN bagi statis $5
                        margin_used = _active_margins.pop(symbol, None)
                        if not margin_used:
                            margin_used = GOLD_MARGIN if symbol in GOLD_SET else BASE_MARGIN
                        
                        roe_percent = (pnl_usd / margin_used) * 100 
                        ...
                        if symbol not in VIP_SET and symbol not in GOLD_SET:
                            _coin_escalation_level[symbol] = _coin_escalation_level.get(symbol, 0) + 1
                    ...
                    for symbol in GOLD_PAIRS:
                        if f"{symbol}_LONG" not in active_keys:
                            for tf in GOLD_TFS:
                                ...
                                if sig:
                                    logger.info(f"🏆 RADAR EMAS: Sinyal {symbol} ({tf})")
                ...
                if current_time - _last_heartbeat_time >= 60.0:
                    ...
                    logger.info(f"👀 System OK [BETA v2.1 PRO] | Market: (VIP: {vip_count}/{MAX_VIP} | ALT: {alt_count}/{MAX_ALT} | GOLD: {gold_status})")
