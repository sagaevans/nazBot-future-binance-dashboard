# ==========================================
# nazBot Sniper System [RC v5.4 - REAL ACCOUNT MODE]
# FILE: bot_logic.py
# FUNGSI: Fixed Notional 50 USDT, Smart Limit Maker, Order Upgrading, Fear & Greed,
#         Stochastic GC & Engulfing logic for Gold & ALL Altcoin Timeframes
# ==========================================
...
# ALT_TFS_FAST dan ALT_TFS_SAFE sekarang menggunakan pola universal
...
GOLD_PAIRS = ["PAXGUSDT", "XAUTUSDT"]
GOLD_SET = set(GOLD_PAIRS)
GOLD_TFS = ['15m', '1h', '4h']

BLACKLIST_COINS = {"PONKEUSDT"}
...
# --- LOGIKA UNIVERSAL: STOCHASTIC GC & ENGULFING (UNTUK GOLD & ALTCOIN) ---
def get_stoch_reversal_signal(symbol: str, tf: str) -> Optional[dict]:
    try:
        bars = _api_call(_client.futures_klines, symbol=symbol, interval=tf, limit=250)
        df = pd.DataFrame(bars, columns=['time','open','high','low','close','volume','ct','qv','tr','tb','tq','i'])[['open','high','low','close','volume']].astype(float)

        if len(df) < 200:
            return None

        close, high, low, open_px, volume = df['close'], df['high'], df['low'], df['open'], df['volume']
        ...
        df['vol_ma'] = volume.shift(1).rolling(window=5).mean()
        df.bfill(inplace=True)

        idx_curr, idx_prev1, idx_prev2, idx_prev3 = len(df)-1, len(df)-2, len(df)-3, len(df)-4

        c_close, c_open, c_high, c_low = close.iat[idx_curr], open_px.iat[idx_curr], high.iat[idx_curr], low.iat[idx_curr]
        p1_close, p1_open, p1_high, p1_low = close.iat[idx_prev1], open_px.iat[idx_prev1], high.iat[idx_prev1], low.iat[idx_prev1]

        c_stoch_k, c_stoch_d = stoch_k.iat[idx_curr], stoch_d.iat[idx_curr]
        p1_stoch_k, p1_stoch_d = stoch_k.iat[idx_prev1], stoch_d.iat[idx_prev1]
        p2_stoch_k, p2_stoch_d = stoch_k.iat[idx_prev2], stoch_d.iat[idx_prev2]
        p3_stoch_k, p3_stoch_d = stoch_k.iat[idx_prev3], stoch_d.iat[idx_prev3]

        c_ema200 = ema200.iat[idx_curr]
        c_vol = volume.iat[idx_curr]
        vol_ma5 = df['vol_ma'].iat[idx_curr]

        # --- SETUP LONG (Searah Tren Naik SMC) ---
        if c_close > c_ema200:
            # 1. Logika Stochastic Golden Cross (Bullish)
            is_gc_recent = (
                (p3_stoch_k < p3_stoch_d and p2_stoch_k > p2_stoch_d) or
                (p2_stoch_k < p2_stoch_d and p1_stoch_k > p1_stoch_d) or
                (p1_stoch_k < p1_stoch_d and c_stoch_k > c_stoch_d)
            )
            cond1_bullish = c_close > c_open
            cond1_stoch_level = c_stoch_k > 40
            cond1_stoch_up = c_stoch_k > p1_stoch_k
            cond1_hl = c_low > p1_low
            cond1_vol = c_vol > vol_ma5

            if cond1_bullish and is_gc_recent and cond1_stoch_level and cond1_stoch_up and cond1_hl and cond1_vol:
                return {'side': 'LONG', 'reason': f'Stochastic GC Momentum [{tf}]'}

            # 2. Logika Bullish Engulfing Reversal
            cond2_oversold = c_stoch_k < 20
            cond2_prev_bear = p1_close < p1_open
            cond2_curr_bull = c_close > c_open
            cond2_sweep = c_low < p1_low
            cond2_engulf = c_close > p1_high
            
            if cond2_oversold and cond2_prev_bear and cond2_curr_bull and cond2_sweep and cond2_engulf and cond1_vol:
                return {'side': 'LONG', 'reason': f'Bullish Engulfing Reversal [{tf}]'}

        # --- SETUP SHORT (Searah Tren Turun SMC - Mirror Logic) ---
        elif c_close < c_ema200:
            # 1. Logika Stochastic Dead Cross (Bearish)
            is_dc_recent = (
                (p3_stoch_k > p3_stoch_d and p2_stoch_k < p2_stoch_d) or
                (p2_stoch_k > p2_stoch_d and p1_stoch_k < p1_stoch_d) or
                (p1_stoch_k > p1_stoch_d and c_stoch_k < c_stoch_d)
            )
            cond3_bearish = c_close < c_open
            cond3_stoch_level = c_stoch_k < 60
            cond3_stoch_down = c_stoch_k < p1_stoch_k
            cond3_lh = c_high < p1_high
            cond3_vol = c_vol > vol_ma5

            if cond3_bearish and is_dc_recent and cond3_stoch_level and cond3_stoch_down and cond3_lh and cond3_vol:
                return {'side': 'SHORT', 'reason': f'Stochastic DC Momentum [{tf}]'}

            # 2. Logika Bearish Engulfing Reversal
            cond4_overbought = c_stoch_k > 80
            cond4_prev_bull = p1_close > p1_open
            cond4_curr_bear = c_close < c_open
            cond4_sweep = c_high > p1_high
            cond4_engulf = c_close < p1_low
            
            if cond4_overbought and cond4_prev_bull and cond4_curr_bear and cond4_sweep and cond4_engulf and cond3_vol:
                return {'side': 'SHORT', 'reason': f'Bearish Engulfing Reversal [{tf}]'}

        return None
    except Exception as e:
        return None

# Fungsi get_adaptive_signal tetap dipertahankan KHUSUS untuk VIP Coins
...
# Fungsi ini sekarang menggunakan logika stoch_reversal_signal
def _scan_single_alt(symbol: str, active_keys: List[str], allowed_tfs: List[str]) -> Optional[Tuple[str, dict]]:
    ...
    for tf in allowed_tfs:
        sig = get_stoch_reversal_signal(symbol, tf)
        if sig:
            return (symbol, sig)
    return None
    ...
    try:
        while not _stop_event.is_set():
            try:
                ...
                # --- EKSEKUSI ALT COIN DENGAN LOGIKA BARU ---
                if alt_count < MAX_ALT and not _stop_event.is_set():
                    ...
                    alts = [t['symbol'] for t in sorted(tickers, key=lambda x: float(x['quoteVolume']), reverse=True)
                            ...
                            and t['symbol'] not in GOLD_SET
                            and t['symbol'] not in BLACKLIST_COINS][:TOP_ALT_LIMIT]
                ...
                if current_time - _last_heartbeat_time >= 60.0:
                    update_fear_greed_index()
                    logger.info(f"[HEARTBEAT] System OK [RC v5.4] | F&G: {_fear_greed_score} | VIP: {vip_count}/{MAX_VIP} | ALT: {alt_count}/{MAX_ALT}")
